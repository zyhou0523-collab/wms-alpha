...
export async function mockRequest<T>(config: AxiosRequestConfig): Promise<T> {
  ...
  if (url === '/inbound-orders' && method === 'get') {
    ...
    const rows = filterInboundOrders(params)
  ...
  if (url === '/outbound-orders' && method === 'get') {
    const params = (config.params || {}) as Row
    return page(filterOutboundOrders(params), Number(params.pageNum || 1), Number(params.pageSize || 10)) as T
  }
  ...
  if (url === '/inventory' && method === 'get') return page(inventoryRows, 1, Number((config.params as any)?.pageSize || 10)) as T
